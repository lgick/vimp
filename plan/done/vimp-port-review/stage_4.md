# Этап 4 ✅ выполнен. Чистка: мёртвый экспорт, лишние аллокации, неверные комментарии
...
- [x] по 4.1 принято явное решение (удалить/покрыть тестом), доки приведены
      в соответствие;
- [x] мёртвая строка в `begin_reconcile` удалена;
- [x] обратный индекс `id → ширина` в `write_hot`;
- [x] комментарии (а), (б), (в) соответствуют коду, (а) исправлен во всех
      трёх местах (код + en + ru);
- [x] все прогоны зелёные, поведение не изменилось.
