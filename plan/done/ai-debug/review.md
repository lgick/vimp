...
## Повторная проверка правок (2026-08-05) — ✅ и остатки закрыты

Правки сверены прогонами, а не только чтением диффов: пересобранный тарбол
игры (`npm pack` → распаковка → `vimp-sim --game <пакет> --determinism`) даёт
12 pass / 0 fail — находка 1 закрыта в том самом сценарии, в котором ломалась.
Эффект правки 2 виден в дампе: `scene-1800.json` больше не тащит сущности
прошлого раунда (было `c1 0, m1 2, w2 0, w2e 1, w1 1` → стало `c1 0, m1 2,
w1 1`), а `report.json` того же прогона похудел с 83 КБ до 8,8 КБ (правка 4).
...
три сценария танков с `--determinism` — 11/11/12 pass, 0 fail.
