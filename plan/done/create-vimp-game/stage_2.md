# Этап 2. Пакет `packages/create-vimp-game`: CLI и генератор ✅ выполнен
...
- [x] `npm create vimp-game` (через `npm run create:game`) разворачивает
      дерево шаблона в пустой каталог и печатает next steps.
- [x] `npx eslint .` зелёный (новый блок `files` и `ignores` на месте).
- [x] `npm test` зелёный, новый проект тестов подхватывается.
- [x] Пины `vimp-engine`/`vimp-engine-core` в сгенерированном проекте
