...
### P1-1. Каркас интерфейса в контейнере SDK не работает: чёрный экран ✅ выполнен
...
> **Уточнено в [review-2.md](review-2.md) (R2-1).** Сохранённая в `style.css`
> форма `body > *` гасила первый уровень страницы, встраивающей SDK (не только
> экраны движка), а совет «вложенному контейнеру задайте свой `display`» ничего
> не лечил: скрыт был предок первого уровня. Итог: в `style.css` осталась
> только форма `.vimp-shell > *`, форма `body > *` вернулась инлайном в
> `index.html` (только FOUC), исключение `body > .vimp-shell` удалено.
...
### P1-2. Окно голосования в solo невидимо: `#vote` монтируется в `document.body` ✅ выполнен
...
### P1-3. Dedicated-сервер падает от одной сорванной сокет-связи ✅ выполнен
...
### P1-4. Remote DoS: длинный заголовок `Origin` валит процесс ✅ выполнен
...
### P2-1. SDK: `playerName` без `playerModel` ломает авто-вход ✅ выполнен
...
### P2-2. `HostGame.destroy()`: двойная синхронизация профиля и гонка на выходе ✅ выполнен
...
### P2-3. Публичный WebSocket dedicated-сервера не защищён от простого абьюза ✅ выполнен
...
### P2-4. `InlineHostBridge`: `destroy()` до `ready` оставляет живой хост во вкладке ✅ выполнен
...
1. **Шаг R1 — блокеры клиента (P1-1, P1-2). ✅ выполнен**
...
3. **Шаг R3 — значимые (P2-1…P2-4). ✅ выполнен**
   ...
   `dedicated.md` (лимиты).
4. **Шаг R4 — мелкие (P3-1…P3-6). ✅ выполнен** Одна пачка, тесты только для P3-2 и P3-4.
5. **Шаг R5 — сборка и smoke.** ⏳ частично: автоматика пройдена —
   `npx eslint .`, `npm test` (122 файла / 1212 тестов), `npm run sim:check`
   (9 инвариантов, 0 падений), `npm run build:app` (CSP-хэш importmap
   совпал). **Не выполнено:** `docker build` (docker на машине недоступен) и
   все четыре ручных прогона из раздела «Верификация» — они требуют браузера
   и собранного пакета игры, их делает разработчик.
