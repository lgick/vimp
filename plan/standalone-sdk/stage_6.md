# Этап 6: Доработки в репозитории игры (`vimp-tanks`)
...
`display` контейнеру задавать не требуется: движок ставит на него класс
`vimp-shell`, а `style.css` выводит контейнер первого уровня из-под правила
скрытия экранов (`body > .vimp-shell { display: revert }`) — см. P1-1 в
[review.md](review.md).
