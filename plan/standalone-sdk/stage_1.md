# Этап 1: Изоморфная порт-машина хоста + стратегии идентичности ✅ выполнен
...
- `tests/host/portMachine.test.js` (новый; прецедент харнеса —
  ...
  - невалидный ник → участник создан под `Player_xxxx`.
    **Решение по итогам ревью (P3-5 в [review.md](review.md)): пункт снят.**
    Путь недостижим — `validateAuth` отбивает невалидный ник раньше, чем
    отработает `resolve`, и это правильное поведение: молчаливая подмена ника
    хуже явной ошибки формы, поле объявлено `required`. Заглушка остаётся
    страховкой для стратегий без поля формы (это записано в её JSDoc) и
    покрыта тестом «гостевая стратегия даёт ник-заглушку, если ник не дошёл».
    Отдельно на будущее: `String(socketId).slice(0, 4)` на UUID даёт 4
    hex-символа (65 536 вариантов) — если путь станет достижимым, поднять
    длину до 6;
