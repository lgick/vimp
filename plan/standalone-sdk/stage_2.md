...
- **Перенести базовые правила из `packages/engine/index.html:12-33` в
  ...
  **Отклонение, закрытое в [review.md](review.md) (P1-1):** перенос был
  ...
  этим правилом скрывался — чёрный экран. Теперь правило имеет две формы
  (`body > *, .vimp-shell > *`), класс-маркер ставит `ensureGameShell`, а
  контейнер первого уровня выведен из-под правила
  (`body > .vimp-shell { display: revert }`).
