...
## B1 (существенное). `parseUnreleased` не знает про блоки кода — контракт обходится молча
...
## B2 (существенное). Записи без под-заголовка проходят как patch
...
## B3 (важное, отступление от плана). `SECTION_LEVELS`: значения не читаются
...
## B4 (важное). Проводка «проблема → preflight» не покрыта тестом, пути продублированы
...
## B5 (среднее). Порядок в `main()`: интерактив до самой дешёвой проверки
...
## B6 (среднее). Тесты: фикстура противоречит новому контракту
...
## B8. `CLAUDE.md`: цель «≤1000 токенов» измерена заниженной эвристикой
...
1. B1 + B2 — один заход по `parseUnreleased`/`validateUnreleased` с тестами:
   без них контракт держится на том, что в журналах не встречаются блоки
   кода.
2. B3 — карта становится движком `suggestLevel`.
3. B4 — `problems` собираются в `decide`, `release.js` худеет, появляется
   тест на проводку.
4. B6, B7 — косметика и фикстуры.
5. B5, B8 — по желанию.
