# Этап 3. Шаблон: инфраструктура сборки и dev-харнес
...
- [ ] Генерация во временный каталог + `npm install` проходит.
- [ ] `npm run build` даёт `dist/` с `manifest.json`, обоими бандлами,
      `maps/`, `sounds/` (ядро на этом этапе — заглушка).
- [ ] `npm run check:contract` проходит группы A и E без `fail`.
- [ ] `npx eslint .` внутри сгенерированной игры зелёный.
