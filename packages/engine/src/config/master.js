...
export default {
  ...
  //
  // Массив и переменная окружения GAMES_MATRIX (JSON) остаются
  // переопределением для двух случаев: локальная разработка (каталог вдобавок
  // дополняется собранными пакетами @vimp-games/* из node_modules,
  // master/localGames.js — прилинкованная игра важнее реестра) и self-hosted
  // мастер без реестра. `package` — имя npm-пакета игры, резолвится через
