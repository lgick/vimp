...
class RoundManager {
  ...
  createMap() {
    ...
    // синхронизация накопленных профилей на мастер перед сменой карты
    // (Этап B4) — естественная граница жизненного цикла. Смена карты
    // заканчивает игру каждого участника (snakes-v3 этап 3): без этого у
    // игры с раундами дневной максимум навсегда остался бы нулём
  ...
  //
  // Подменяются ОБЕ копии, и это не удобство, а исправление: `sendMap`
  // отдаёт подключающемуся `_currentMapData`, то есть карту каталога — и
  // клиент рисовал геометрию, которой в ядре уже нет, пока игра сама не
  // пришлёт ему MAP_DATA. Между этими двумя кадрами он видит чужую границу.
  //
  // `mapData` приходит в КЛИЕНТСКОМ виде (несмасштабированные координаты плюс
  // `scale`) — это тот же объект, который игра рассылает через `sendMap`, —
  // поэтому масштабирование делается здесь, ровно как в `_prepareMapData`.
  // Отдать `_scaledMapData` клиенту напрямую нельзя: клиент масштабирует сам
  // (`client/main.js`, applyMapData), и при `scale !== 1` карта была бы
  // умножена дважды.
  ...
  _checkTeamWipe(victimTeamId, killerTeamId) {
    ...
    // (Этап B4) — естественная граница жизненного цикла. Конец раунда и
    // есть конец игры участника (snakes-v3 этап 3)
    this._playerDataSync?.finishAllGames?.();
    this._playerDataSync?.flushAll();
