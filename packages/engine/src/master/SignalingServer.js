...
export default class SignalingServer {
  ...
  async _onRegisterHost(session, { name, maxPlayers, mapName, gameId, gameVersion, token }) {
    ...
    const host = this._registry.add({
      ...
      //
      // Значение считается ОДИН РАЗ, при регистрации, и после одобрения
      // версии тестовая комната остаётся скрытой до перерегистрации хоста.
      // Для текущего масштаба это приемлемо: одобрение — редкое админское
