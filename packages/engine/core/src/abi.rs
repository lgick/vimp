...
/// Имя опкода читается ТОЛЬКО отсюда — как на JS-стороне из
/// `src/config/abiOps.js` (`ABI_OP_DEBUG_JSON`). Литерал, написанный руками
/// и в списке, и в `match` обработчика, разъезжается молча: ядро объявляет
/// опкод, которого не обрабатывает, движок видит «не обработан» и уходит на
/// запасной путь, ничего не сказав.
pub const OP_DEBUG_JSON: &str = "debug.json";
...
pub const ENGINE_GAME_OPS: &[&str] = &[OP_DEBUG_JSON];

/// То же для клиентского макроса.
pub const ENGINE_CLIENT_OPS: &[&str] = &[OP_DEBUG_JSON];
