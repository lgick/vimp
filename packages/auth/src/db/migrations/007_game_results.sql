...
--
-- Условие `best = 0 AND delta > 0` описывает старую строку не само по себе, а
-- лишь потому, что новая такой не бывает: движок закрывает игру только с
-- положительным счётом (PlayerDataSync.finishGame), а best там максимум среди
-- просуммированных игр, то есть тоже > 0. Держится это на инварианте
-- движка, а не на самом WHERE, — поэтому повторный ручной прогон миграции
-- ПОСЛЕ появления новых данных безопасен ровно до тех пор, пока инвариант
-- цел. Штатно миграции применяются один раз, по журналу.
