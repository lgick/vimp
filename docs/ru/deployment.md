...
- `dedicatedGame` — id игры, при желании с пином точной версии пакета (`"tanks"` или `"tanks@0.16.1"`). Если поле есть, `deploy.yml` пишет `VIMP_DEDICATED_GAME` в `.env.prod` этого сервера, и `src/master/main.js` стартует dedicated-сервером вместо лобби; если поля нет, бокс остаётся лобби-мастером. Остальное в матрице не меняется.
- Игра разрешается так же, как каталог лобби-мастера: прилинкованный в `node_modules` пакет важнее, иначе сервер спрашивает реестр (`VIMP_AUTH_SERVICE_URL`) и качает пакет в `VIMP_GAMES_DIR` сам — поэтому **dedicated-боксу нужен тот же том**. Если недоступно ни то, ни другое, процесс падает с именованной ошибкой, называющей оба способа, см. [dedicated.md](dedicated.md).
...
`master:games` / `GAMES_MATRIX` остаётся статическим переопределением для
локальной разработки и self-hosted мастера без реестра — см.
