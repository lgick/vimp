...
1. Разработчик публикует пакет в npm и подаёт заявку из лобби («Мои игры» →
...
2. Админ открывает «Модерацию», при желании жмёт «Тест» (версия качается и
...
3. Новая версия уже одобренной игры идёт тем же путём («Мои игры» →
   «Обновить» → модерация) и тоже ничего не деплоит.
