...
Игры с `gameConfig.noSpectators` это не касается: наблюдателей в них нет,
участник входит сразу в играющую команду и актора получает на первом кадре
сам. `startupVotes` для такой игры оставляйте пустыми — отвечать нечему, а
`['teamChange', '<та же команда>']` даст в чат «это ваша текущая команда».
