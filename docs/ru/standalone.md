...
`.vimp-shell > *`, чтобы экраны движка не показались разом — каждый из них
показывает свой модуль. Сам контейнер не скрывает ничто и ни на какой глубине
вложенности, `display` от страницы ему не нужен. Учтите, что правило — селектор
по классу: собственный элемент на первом уровне контейнера, который показан
правилом по типу или классу (`canvas { display: block }`), правилу проиграет —
целитесь в него по id.

`style.css` — таблица уровня страницы, а не изолированная: вместе с SDK
встраивающая страница получает `html, body { width: 100%; height: 100% }`,
движковые фон, цвет и шрифт `body`, а также `* { user-select: none }` — то
есть выделение текста пропадает на всей странице, а не только внутри
контейнера. Страницу стоит отдать игре целиком.
...
- **Чёрный экран при явно идущем матче** (звук играет, ошибок нет) — сверху
  лежит собственный элемент контейнера, либо CSS игры показывает что-то на
  первом уровне контейнера правилом слабее `.vimp-shell > *` (см. выше) и
  движок держит это скрытым.
