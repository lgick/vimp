...
Игры **не** зависимости этого репозитория: в проде каталог приезжает из
реестра игр central auth-сервиса, и мастер качает одобренные пакеты сам (см.
[master.md](master.md#get-gamesmanifestjson-get-gamesid-get-gamesidversion)).
Локально реестр не мешает: прилинкованная в `node_modules` игра всегда
важнее записи реестра с тем же id — именно это сохраняет HMR исходников самой
игры.
...
### Два других локальных запуска

Лобби выше — один из трёх способов получить матч на этой машине:

| Запуск | Команда | Что получается |
| --- | --- | --- |
| лобби-мастер | `npm run dev` | комнаты, каталоги, WebRTC-сигналинг, OAuth — прод-поведение |
| dedicated-сервер | `VIMP_DEDICATED_GAME=tanks npm run dedicated` | один матч внутри процесса Node, `http://localhost:3002`, без лобби и OAuth — [dedicated.md](dedicated.md) |
| standalone SDK | `npm run dev` **в репозитории игры** | весь матч в одной вкладке, вообще без наших серверов — [standalone.md](standalone.md) |

Последним двум auth-сервис не нужен. И dedicated-сервер, и headless-раннер грузят **node**-сборку ядра игры (`dist/core-node/`), поэтому слинкованный чекаут игры обязан быть собран через `npm run core:build:node`, а не только веб-ядром.
