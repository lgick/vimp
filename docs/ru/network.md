...
| 18 | `ACCOLADES_DATA` | JSON | Места участников комнаты в глобальном топе, формируются `host/meta/modules/Accolades.js` и применяются на клиенте `lib/accolades.js` |
...
Поле вправе объявить и `role` — свою роль в движковом контракте (в отличие от `name`, которое принадлежит игре). Ролей сегодня две, и они ходят парой в блоке динамики карты: `role: 'z'` на позиции 3 и `role: 'level'` на позиции 4 делают строку динамики слоёной (высота тела над своим уровнем и сам уровень). Раньше движок узнавал эту пару по *именам* полей, и переименование поля молча возвращало плоскую строку; теперь авторитет — только роли, а загрузка карты падает с ошибкой, если роль стоит не на своей позиции, если объявлена лишь одна из пары или если на этих двух местах стоят поля с именами `z`/`level` без ролей.
