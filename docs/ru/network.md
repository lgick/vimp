...
Третья роль, `role: 'state'`, — байт состояния тела карты: `u8`, который движок пишет сразу за головой строки — на позиции 5 у слоёной строки, 3 у плоской — и до необязательного хвоста скоростей, поэтому `optionalFrom` обязан быть больше его позиции (покоящееся тело хвост не шлёт и потеряло бы байт). Строка становится `[x, y, angle, z?, level?, state, vx?, vy?, angvel?]`. Значение даёт игра (`SimCtx::map_body_state`), смысл значений — её. Схема без этой роли сохраняет прежнюю строку бит-в-бит; роль не на своей позиции или не того типа роняет загрузку карты.
