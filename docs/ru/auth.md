...
`VIMP_ADMIN_IDENTITIES` — более безопасный, тоже необязательный источник
того же права: CSV из пар `провайдер:uid` (`github:1234567`), сравниваемых
со столбцами `provider` / `provider_uid` строки `users` (у GitHub это
числовой id аккаунта, он переживает переименование). **Она полностью
перебивает `VIMP_ADMIN_NICKS`**: пока она непуста, ник не даёт ничего и
`admin` получает только перечисленная личность (`isEnvAdmin` в
`src/lib/adminRights.js`); если она не задана, список ников работает
ровно как раньше.

**Зачем она нужна.** Права, привязанные к *строке*, безопасны ровно
настолько, насколько эта строка занята. Ник из `VIMP_ADMIN_NICKS`, которого
ещё нет в `users` — новая БД, ник внесён в список до первого входа
владельца, опечатка в списке, — достаётся первому, кто под ним
зарегистрируется, вместе с `role = 'admin'`; OAuth-вход открыт всем, и
гонка выигрывается одним запросом. Занять ник *уже зарегистрированного*
админа нельзя (ник глобально уникален без учёта регистра, переименования не
существует), так что уязвимо ровно окно свободного ника.

**Рекомендация для новой установки**: либо занять админские ники до
открытия регистрации, либо сразу заполнить `VIMP_ADMIN_IDENTITIES`. При
старте сервис печатает по строке на каждый админский ник — предупреждение
для незанятого и `[admin] "<ник>" -> github:<uid>` для существующего, и это
готовое значение для `VIMP_ADMIN_IDENTITIES` (лезть SQL-ом в прод не
нужно). Обе строки не печатаются, если `VIMP_ADMIN_IDENTITIES` задана.
...
`users.role` по умолчанию `'user'`, ролей всего две — `'user'` и
`'admin'`. Единственный источник админских прав — окружение:
`VIMP_ADMIN_IDENTITIES`, если она задана, иначе `VIMP_ADMIN_NICKS`.
`syncRole` записывает роль при каждом выпуске токена одним запросом, и его
`CASE` понижает любого `admin`, которого нет в списке, — назначить админа в
обход списка правкой строки в БД нельзя. Прочие роли (будущий модератор из
админки) он не трогает.
...
| `period` | Читается из | Окно |
| --- | --- | --- |
| `day` | `rank_periods.best` (kind `'d'`) | календарные сутки UTC, в реальном времени |
| `month` | `rank_periods.points` (kind `'m'`) | календарный месяц UTC, в реальном времени |
| `all` | кэш `ratings` | снимок, пересчёт раз в сутки |

**Два живых среза читают агрегат, а не леджер**
(`008_rank_period_aggregates.sql`). Одна строка на (игрока, игру, окно):
игра, пишущая полмиллиона строк леджера в сутки, читается как восемь тысяч —
число её игроков, — а топ и место становятся диапазонным сканом индекса
вместо хеш-агрегата по всем суткам. `recordGameResult` ведёт агрегат ТЕМ ЖЕ
запросом, которым дописывает строку леджера, поэтому разойтись они не могут, а
окна берутся от `created_at` самой этой строки, а не от второго `now()`,
который на границе суток попал бы в другие сутки. Источником истины остаётся
леджер: `recomputePeriods` пересобирает агрегат из него при аннулировании
вклада хостера — `best` это максимум, и вычесть из него нельзя.

**Место — не запрос на каждого игрока.** «Сколько впереди меня» — это один и
тот же ответ для всех, поэтому он не спрашивается персонально.
`RankDistribution` (`src/db/RankDistribution.js`) кэширует на (игру, срез) и
на `rank.distributionTtl` лестницу РАЗЛИЧНЫХ значений среза вместе с числом
игроков «на этой ступени и выше»; место после этого — бинарный поиск, а своё
значение — точечный поиск по первичному ключу. Определение не изменилось:
`1 + (игроков строго выше)` — это в точности `RANK()` из `/leaderboard`, так
что плашка и список по-прежнему согласованы.

Лестница хранит различные значения, а не игроков, поэтому её память не растёт
вместе с размером игры; она обрезается на `rank.distributionSteps` ступенях, а
игра, которая в них не уместилась, отвечает глубокому хвосту точным запросом.
Своё значение читается живым всегда, поэтому устареть может только ОКРУЖЕНИЕ и
не дольше TTL — причём только в одну сторону, так как `best` и `points` только
растут.

`all` намеренно не пересчитывается на записи: `PUT /rank` — это один запрос
...
`npm -w @vimp/auth run db:ratings`.

Прогон берёт **сессионную консультативную блокировку**
(`pg_try_advisory_lock`) на отдельном соединении и пропускает себя, если замок
занят. Это не уборка: запрос инкрементный (`прошлый rank + SUM новых
событий`), и два наложившихся прогона не просто сделали бы лишнюю работу, а
УДВОИЛИ бы каждому игроку сутки — второй читает то же самое
`ratings.updated_at`, потому что первый ещё не закоммитил своё `now()`. А
наложить их несложно: `startRatingsJob` запускается в каждом процессе auth,
ручной `db:ratings` может совпасть с плановым, а перезапуск — попасть в окно.
Следствие суточного ритма тоже стоит проговорить: и список за всё время, и
своя строка в нём показывают снимок на 00:00 UTC, сегодняшних игр в нём
нет.
...
| `GET /games` (без авторизации — master-game-registry) | `{ games: [...] }` — каталог, из которого мастер строит свой список: одобренные игры с раздаваемой `version`, сортировка по `id` (порядок обязан быть детерминированным — первая игра становится активной в лобби). Публичный по той же причине, что и `/leaderboard`: список игр платформы и так виден в лобби, — поэтому строка урезается до `id`, `packageName`, `title`, `repoUrl`, `authorNick`, `version`, `maxGameScore`, а замечание модератора, внутренний id автора и заявленная версия наружу не едут |
| `GET /games/mine` (Bearer identity-токен) | собственные заявки вызывающего со `status`, `pendingVersion` и `moderatorNote`, но **без** `moderatorNick` (снимает `src/lib/gameViews.js`; так же отвечают и авторские `POST /games`, `POST /games/:id/version`). Модерация со стороны автора анонимна: ему адресовано замечание, а не личность — «эту заявку зарубил вот этот человек» это продуктовое решение про травлю модераторов, а не побочный эффект общей проекции строки. По той же причине ника нет и в `GET /games` |
| `POST /games` (Bearer, `{ id, packageName, title?, repoUrl?, version }`) | заявка любого авторизованного пользователя — роли `developer` нет: заявитель становится автором игры (`author_user_id`), передать авторство другому вправе только админ (`PATCH /admin/games/:id`, `authorNick`). `201 { game }`, создаётся в `pending` с `pendingVersion = version`. Rate-limit по IP (5/60с). Отсутствие `id`, `packageName` или `version` — `400 { error: 'badRequest', field }`: обязательность полей принадлежит этому роуту, тогда как проверки формата ниже общие с модерацией. Каждое поле отвечает своим кодом — `400 invalidGameId` \| `invalidPackageName` \| `invalidVersion` \| `invalidTitle` \| `invalidRepoUrl`; `409 gameExists` на занятый `id` или уже зарегистрированный пакет (и то и другое — один `23505`), `403 tooManyGames` сверх `config.games.maxPerUser` |
...
| `DELETE /games/:id` (Bearer — автор или админ) | удаляет игру из реестра **вместе со всеми строками по её `game_id`** — `rank_periods`, `rank_events`, `state_snapshots`, `states`, `ratings` именно в этом порядке, строка `games` последней. FK на `games` нет ни у одной из этих таблиц, поэтому чистка явная: осиротевшие строки воскресли бы вместе с игрой, заведённой позже под тем же id. Транзакцией не обёрнуто (их не держит нигде весь репозиторий) — порядок выбран так, чтобы прерванный прогон было безопасно повторить, а игра до последнего шага оставалась видимой. Админ удаляет игру в любом статусе, автор — только свою и только пока её не раздают: `409 gamePublished` на `approved` (сначала админ снимает её с раздачи), `403 forbidden` на чужую, `404 unknownGame`, если игры нет. Rate-limit по IP общий с остальными записями реестра (5/60с) |
| `GET /admin/games` (Bearer, админ) | вся очередь модерации, свежее сверху, с никами автора и модератора |
| `PATCH /admin/games/:id` (Bearer, админ, `{ status?, version?, pendingVersion?, note?, maxGameScore?, authorNick? }`) | решение модератора — записываются только переданные ключи. `status: 'approved'` без явного `version` поднимает `pendingVersion` в `version` и обнуляет его (самый частый путь не должен требовать двух полей). `maxGameScore` обязан быть положительным целым в пределах собственных потолков сервиса (`<= config.rank.maxGameScore`, и `× 20` — окно склейки движка — по-прежнему в пределах `config.rank.maxPoints`), иначе `400 invalidMaxGameScore`: потолок выше них позволил бы мастеру пропустить результат, который этот сервис затем отклонит, и хост повторял бы этот flush вечно. `404 unknownGame`. **`authorNick`** переназначает авторство: ник резолвится в id пользователя без учёта регистра (`400 badRequest` — ник невалиден, `404 unknownUser` — такого нет), `null` и `''` снимают автора — игра платформы законно ничья. Это единственный способ дать автора играм, засеянным `009_games.sql` с `author_user_id = NULL`: до этого они не попадают ни в чей «My games» и запросить для них новую версию некому. Если решение оставило платформу вовсе без раздаваемой игры, в ответ добавляется `warning: 'catalogEmpty'` — это не отказ (лобби остаётся рабочим: вход, «My games» и «Moderation» от игры не зависят), но создавать комнаты станет не на чем, пока не опубликуют новую, и узнать об этом модератор должен здесь, а не от игроков |
...
3. **Синхронизация обратно**: `PlayerDataSync.flush()`/`flushAll()`
   ...
   одной синхронизации на участника за `minFlushInterval` (300 с, с джиттером
...
`VIMP_ADMIN_NICKS` и `VIMP_ADMIN_IDENTITIES` — регистр, пробелы, хвостовая
запятая, пустое значение, мусорная пара), `adminRights.test.js` (`isEnvAdmin`
— список личностей перебивает список ников, чужой uid и отсутствующий
провайдер отказывают fail-closed),
