...
`coreParams` — необязательный непрозрачный словарь **собственных**
параметров ядра игры. `buildCoreConfig` вливает его в игровую половину
конфига ядра (`hostPlugin.createCore(...)`), причём известные движку ключи
(`friendlyFire`, `models`, `weapons`, `playerKeys`, `panel`) перекрывают
одноимённые — подменить движковую часть контракта игра не может. Остальное
движок не читает и не валидирует, он лишь довозит его до `GameSim::new`:
именно поэтому новый параметр игрового ядра больше не требует релиза
движка (`vimp-tanks`, например, передаёт так параметры падения 2.5D).

`mapFallTime` — сколько секунд длится падение на уровень высоты (по
умолчанию 0.35). В отличие от `coreParams` это **движковый** ключ:
`buildCoreConfig` кладёт его в половину `engine`, потому что траектория
(`FallModel` в ядре) общая для всего, что роняет игра, и для динамических
тел самой карты — их шагает движок. Тело, под которым не осталось плиты,
падает на ближайшую плиту снизу.

У игры со своими уровнями это число обычно уже есть — в
`coreParams.levels.fallTime`: по нему её ядро роняет танки и предсказывает
на клиенте те же самые ящики. Два независимых поля роняли бы ящик у хоста и
у его же клиента с разной скоростью, и разошлись бы они молча, поэтому
`buildCoreConfig` берёт `coreParams.levels.fallTime` (если это конечное
положительное число) ещё и как `mapFallTime`. Движковый ключ остаётся
умолчанием для игр без уровней, а явный override по-прежнему сильнее обоих.

`maps[].levelHeight` — высота одного уровня в мировых единицах (без
масштаба: масштаб карты умножает её ядро; по умолчанию — размер тайла). Она
делает уклон рампы безразмерным — `rise * levelHeight / span` вместо
«уровней на пиксель», — и константы подъёма (тяга, наклон корпуса, пыль)
снова подбираются как уклон. Проверяют ядро и правило **E4**: конечное
число больше нуля.

`maps[].volumes` (и `maps[].levels[n].volumes`) — необязательный словарь
`{ "<ключ layers>": высота }`: визуальная высота рендер-слоя в уровнях. Его
проверяют ядро и правило контракта **E4** (ключ обязан называть рендер-слой
того же уровня, высота — лежать в `(0, 8]`), через хост он едет нетронутым и
доезжает до рендер-парта полем `volume`; геометрию строит игра.
...
`capacity` (кольцевой буфер), `angles` (индексы компонентов-углов, сравниваемых
по окружности). См.
...
[client.md](client.md#soundmanager). Там же описан необязательный блок
`parts.sounds.spatial` (профиль проекции, виртуальная высота, радиус
рассеивания, атрибуты `PannerNode`): игра, которая его не объявила,
получает движковые дефолты. Переменных `.env` задача не вводит.
