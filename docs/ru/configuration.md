...
| `VIMP_DEDICATED_GAME` | игра dedicated-сервера — `<id>` либо `<id>@<version>`; если задана, `src/master/main.js` поднимает [dedicated-сервер](dedicated.md) вместо лобби-мастера | — |
...
| `GAMES_MATRIX` | JSON-массив, переопределяющий `master:games` (**статический** список игр, `{id, package}[]`), читается и в dev — см. [master.md](master.md#get-gamesmanifestjson-get-gamesid-get-gamesidversion) | `[{"id":"tanks","package":"@vimp-games/tanks"}]` |

`GAMES_MATRIX` больше не способ задать прод-каталог: игры приезжают из
реестра central auth-сервиса, и мастер качает их сам. Переменная остаётся
переопределением для двух случаев — локальная разработка и self-hosted мастер
без реестра.

Вне прода каталог ещё и **собирается сам**: если `GAMES_MATRIX` не задана, в `master:games` добавляется каждый собранный пакет `@vimp-games/*`, найденный в `node_modules` (обычной зависимостью или симлинком `npm link`) — по возрастанию id и впереди записей конфига (`src/master/localGames.js`). Прилинкованная игра появляется в лобби без правки опубликованного конфига движка и **важнее записи реестра с тем же id** — именно это делает возможной HMR-разработку самой игры. Первая запись каталога — активная игра лобби; чтобы закрепить, какая именно, задайте `GAMES_MATRIX` локально.
