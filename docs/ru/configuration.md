...
модерируются в лобби, живут в Postgres auth-сервиса, а одобренные пакеты
мастер качает сам. Пустой реестр означает пустое лобби — это законное
состояние, а не ошибка настройки.

Вне прода каталог ещё и **собирается сам**: в `master:games` добавляется каждый собранный пакет `@vimp-games/*`, найденный в `node_modules` (обычной зависимостью или симлинком `npm link`) — по возрастанию id и впереди записей конфига (`src/master/localGames.js`). Прилинкованная игра появляется в лобби без правки опубликованного конфига движка и **важнее записи реестра с тем же id** — именно это делает возможной HMR-разработку самой игры. Первая запись каталога — активная игра лобби. В проде обнаружение выключено: пакет, случайно попавший в образ, не должен перекрывать версию, прошедшую модерацию.
...
- `games` — **статический** список игр, `{id, package}[]`, **по умолчанию пуст**. Штатный источник каталога — реестр игр central auth-сервиса, откуда мастер сам качает одобренные пакеты (`GameSync`). Env-переопределения у массива нет, и остаётся он двум потребителям: локальной разработке, где достраивается из `node_modules`, и [dedicated-серверу](dedicated.md), который ищет здесь свою игру прежде, чем идти в реестр. `package` резолвится как обычная зависимость `node_modules/` (публикуется собственным репозиторием игры, например `vimp-tanks`), поэтому версию плагина задаёт установленная зависимость, а не этот список. У записи может быть и поле **`maxGameScore`** (snakes-v3) — потолок результата ОДНОЙ игры этой игры, которым мастер клампит `best`/`points` в `PUT /auth/rank`. Для игр реестра его задаёт админ при модерации, здесь — только запасной путь. Не задан — применяется `master:playerData:maxGameScore`: рабочий предел именно пер-игровой, потому что один точный предел на сотни игр неверен по построению;
- `gameStore` — хранилище игровых пакетов (мастер качает одобренные игры из npm registry и раздаёт их с диска, вместо того чтобы получать их npm-зависимостью на этапе сборки образа):
  - `dir: null` — корень хранилища; `null` → `<repoRoot>/.games`. В проде задаётся `VIMP_GAMES_DIR` и монтируется томом;
  - `registryUrl: 'https://registry.npmjs.org'` — npm registry;
  - `refreshInterval: 60000` — период опроса реестра auth за изменениями каталога, мс;
  - `maxTarballBytes: 67108864`, `maxFiles: 5000` — потолки распаковки недоверенного архива;
  - `keepVersions: 2` — сколько **раздаваемых** версий одной игры держать на диске. Черновик админа («Test») держится сверх этого потолка: он существует только на диске, тогда как раздаваемую версию всегда можно перекачать из npm;
  - `timeout: 30000` — потолок ожидания ответа реестра, мс;
...
- `playerData` — потолок записи профилей (snakes-v3, «игр сотни, серверов сотни»): `writesPerMinute: 120` — `PUT /auth/rank` + `PUT /auth/state` на **проверенную комнату** в минуту, сверх этого `429`. Честная комната на 32 при пятиминутном интервале пишет ~13 в минуту, остальное — запас на срочные границы (уход участника интервал обходит); потолок держит сломанную или злонамеренную комнату, поэтому запас считается от честной. И `maxGameScore: 10000` — дефолтный потолок результата одной игры для игры, не объявившей свой. Минимальный интервал между записями держит сторона хоста (`lobbyConfig.playerData`), а этот блок останавливает сломанный или злонамеренный сервер, который его обошёл;
...
- `playerData` — всё, что нужно `PlayerDataSync`, и движковый ответ на вопрос «как часто комнате позволено писать в базу» (snakes-v3). Эндпоинты: `rankUrl: '/auth/rank'` (`PUT` результата игры `{ points, best }`), `stateUrl: '/auth/state'`, `placementsUrl: '/auth/placements'` (агрегирующий роут — все три среза за один поход на входе) и `placementUrl: '/auth/placement'` (перезапрос одного среза из `refreshPlacement`). Бюджет: `minFlushInterval: 300000` мс на участника, `flushJitter: 0.2` (±20 % на комнату, чтобы сотни серверов не писали в одну и ту же секунду), `maxRequestsPerSecond: 1` (очередь запросов комнаты — держится строго ниже потолка мастера, `master:playerData:writesPerMinute / 60 = 2/с`, чтобы очередь тормозила сама, а не узнавала свой предел из `429`, который стоит round-trip'а и роняет в бэкофф всю комнату), `backoff: { baseMs: 30000, maxMs: 900000 }` (экспоненциальный откат комнаты на `5xx`/`429`/сетевых сбоях; обе границы соразмерны `minFlushInterval` — пауза КОРОЧЕ интервала не отложила бы ни одного запроса и делала бы бэкофф мёртвым кодом) и `placementTtl: 30000` (троттлинг `refreshPlacement`).

  **Откуда пять минут.** Целевой масштаб — 100 игр × 100 серверов × 8 игроков = 80 000 игроков одновременно, а участник со свежим результатом стоит двух записей за интервал: 80 000 × 2 / 60 с — это 2700 записей в секунду, 80 000 × 2 / 300 с — 530. Платится за это свежестью ГЛОБАЛЬНЫХ рейтингов и только ею: результаты склеиваются в памяти комнаты (сумма складывается, максимум берётся максимумом), поэтому не теряется ничего, свои значения игрок видит сразу, а срочные границы интервал по-прежнему обходят. Не изменилось — не отправляем, поэтому тихая комната не пишет вообще ничего; игра запись только *просит*;
