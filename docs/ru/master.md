...
| --- | --- | --- |
| `GET /games/lookup?package=&version=` | любой вошедший | разбор пакета для формы заявки: мастер качает и проверяет его и отдаёт `{id, title, version, versions, repoUrl, engineApi, compat, errors}`. Лимит тот же, что у `submit` — роут ходит за чужим тарболлом |
| `GET /games/mine` | любой вошедший | собственные заявки вызывающего со статусами и замечаниями модератора |
| `POST /games/submit` | любой вошедший | заявка на новую игру: пакет качается и проверяется **до** записи строки, поэтому разработчик получает список проблем сразу, а реестр не засоряется нерабочими заявками. В теле едут только `{packageName, version}` — см. ниже. Лимит — 5 заявок в минуту на пользователя, до похода в npm |
| `POST /games/mine/:id/version` | владелец игры (и админ — по любой игре) | новая версия уже заведённой игры, проверяется так же |
| `DELETE /games/mine/:id` | любой вошедший (решает auth) | удаление игры. Право проверяет auth-сервис, а не путь: админ удаляет любую игру, автор — только свою и только пока её не раздают (иначе `409 gamePublished`). В реестре удаление **мягкое** (строка и рейтинги живут ещё 30 суток), но это дело auth-сервиса: для мастера игра просто пропала из `GET /games`. На `200` запись каталога снимается **целиком** (`catalog.remove(id)` без версии — вместе с застейдженным черновиком админа, снять который больше некому), и тут же гоняется проход синхронизации; файлы версии выметает ближайший `prune` внутри него |
| `GET /admin/games` | `role=admin` | вся очередь модерации — включая мягко удалённые игры, у каждой едут `deletedAt` и `purgeAt`, — плюс локальное состояние каждой игры на этом мастере (`downloaded`, `stagedVersion`, `lastError`) |
...
| `PATCH /admin/games/:id` | `role=admin` | решение модератора (статус, раздаваемая версия, замечание, `maxGameScore`) и автор игры (`authorNick` — поле `Author` панели; пустое поле снимает автора). Мастер проксирует тело как есть: ник резолвится в пользователя auth-сервисом. Авторство и есть то, что кладёт игру в «My games» её автора и разрешает ему запросить новую версию, поэтому засеянные миграцией игры остаются ничьими, пока админ не заполнит это поле. При успехе мастер тут же гоняет проход синхронизации, поэтому админ видит результат немедленно, а остальные мастера подхватывают его в течение `refreshInterval` |
| `POST /admin/games/:id/restore` | `role=admin` | возврат мягко удалённой игры вместе с её рейтингами и скиллами. Мастер ничего не докачивает сам: восстановленная игра снова появляется в `GET /games`, и её забирает проход синхронизации, который роут тут же и гоняет |
| `GET /admin/games/:id/versions` | `role=admin` | что опубликовано в npm для пакета — индикатор «есть версия новее» |

Форма заявки спрашивает **две вещи: npm-пакет и версию**. Остальное мастер
читает с самого пакета: `id`, `title` и резолвнутая версия — из
`dist/manifest.json` внутри тарболла (мастер и так распаковывает его ради
проверки), ссылка на репозиторий — из пакумента npm: `repository` живёт в
`package.json` пакета, а до диска доезжает только `package/dist/**`.
Спрашивать у человека то, что мастер сейчас прочитает и проверит сам, — ровно
то, чем форма занималась раньше.

Поэтому оба роута стейджат пакет, **не зная его id заранее**
(`GameStore.inspectPackage`): распаковка идёт в корневой
`<dir>/.staging/<rand>`, id читается из манифеста после неё и проходит те же
`GAME_ID_PATTERN`/`RESERVED_GAME_IDS`, что и присланный, — манифест
недоверенный, а id становится сегментом URL и именем каталога. Тело с `id`,
`title` и `repoUrl` `submit` по-прежнему принимает (старый клиент, прямой
вызов), но прочитанное из пакета важнее.

Отказ auth-сервиса на любом из них — `502 { "error": "authServiceUnavailable" }`.

Очередь модерации разбита на **графы** кнопками над списком
(`lobby.games.statuses`). Четыре из них — статусы реестра, пятая, `Deleted`,
статусом не является: мягко удалённая игра сохраняет свой прежний статус, а в
графу попадает всё с непустым `deletedAt`. Уходит игра туда *целиком*:
оставить её ещё и в `Pending` или `Published` значило бы предлагать
модерировать то, чего у игроков уже нет. Карточка там не несёт органов
модерации вовсе — только срок полного удаления и `Restore`. Поэтому и
подтверждения вторым нажатием у кнопки удаления больше нет: 30 суток оно
обратимо.

**«My games» и «Moderation» — две страницы одной панели**, а не две карточки
на одном экране: показывается ровно одна из них. Шапка панели (заголовок,
переключатель страниц и «Back to lobby») лежит над карточками и переживает
переключение, поэтому выход в лобби есть на обеих; переключатель виден только
админу, у остальных панель состоит из одной страницы.

**Кнопка «Delete»** стоит в обеих карточках панели — в «My games» у автора и в
очереди модерации у админа, обе дёргают один и тот же роут и срабатывают с
первого нажатия. Модальных `confirm()` в лобби нет ни одного, и заводить их не
стали.
...
- `GET /auth/placement?game=&period=` — нужны Bearer-токен и `?game=`, те же
  случаи `400`/`404`/`502`, что у `/auth/rank`/`/auth/state`, пробрасывается
  в `PlayerDataProxy.getPlacement(token, game)`. Per-user данные, кэшируются
  через `PlacementCache` (`packages/engine/src/master/PlacementCache.js`,
  keyed-TTL, по умолчанию 30с — `master:placement:cacheTtl`), по той же
  схеме, что `LeaderboardCache` перед лидербордом. `period` читается и
  проверяется перед проксированием ровно так же, как у лидерборда.
- `GET /auth/placements?game=` — одним походом агрегирует все три среза
  ранга (`day`/`month`/`all`) для вызывающего, через тот же `PlacementCache`
  по каждому срезу.
