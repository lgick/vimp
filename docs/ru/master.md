...
| `DELETE /games/mine/:id` | любой вошедший (решает auth) | удаление игры. Право проверяет auth-сервис, а не путь: админ удаляет любую игру, автор — только свою и только пока её не раздают (иначе `409 gamePublished`). Мастер лишь убирает за собой: на `200` запись каталога снимается **целиком** (`catalog.remove(id)` без версии — вместе с застейдженным черновиком админа, снять который больше некому), и тут же гоняется проход синхронизации; файлы версии выметает ближайший `prune` внутри него |
...
очереди модерации у админа, обе дёргают один и тот же роут. Подтверждение в два
нажатия: первое переводит кнопку в «Confirm delete», второе отправляет запрос.
Модальных `confirm()` в лобби нет ни одного, и заводить их не стали; перерисовка
списка возвращает кнопку в исходное состояние, поэтому незавершённое
подтверждение её не переживает.
