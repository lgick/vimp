...
В каждый отдаваемый манифест каталог дополнительно кладёт три поля, которых
сборка игры не пишет: `packageName`, `packageVersion` и `packageHomepage` —
вычитанные из собственного `package.json` разрезолвленного пакета (`homepage`,
иначе `repository`). Клиент показывает их в футере формы входа (см.
...
определению. Пакет без читаемого `package.json` оставляет все три поля `null`
и из каталога не выпадает — пустеет только футер. Dedicated-сервер
переиспользует тот же `GameCatalog`, поэтому его футер `#auth` заполняется так
же.
