...
Контейнер **обязан быть полноэкранным и позиционированным** (`position: relative`): `#panel`, `#stat` и `#vote` — `position: absolute`, и их containing block — ближайший позиционированный предок. Видимостью экранов движок занимается сам: `ensureGameShell` ставит на контейнер класс `vimp-shell` (экспортируется как `SHELL_CLASS`), а `style.css` скрывает `body > *, .vimp-shell > *` — каждый экран показывает свой модуль. Сам контейнер выведен из-под правила через `body > .vimp-shell { display: revert }`, поэтому контейнеру первого уровня `display` от страницы не нужен; контейнер, вложенный глубже, страница обязана показать сама (селектор по id перебивает правило).
