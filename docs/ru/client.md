...
- **Карта (`applyMapData`)** отдаёт payload клиентскому ядру (`set_map`) и собирает статические данные рендера. В ядро слоёные поля уходят как приехали — `levels` (надземные уровни: `map`, `floor`, `walls`, `layers`) и `ramps`, — чтобы оно построило ту же геометрию, что и хост: без них предсказание уровня молча разъехалось бы с авторитетным. Данные рендера собираются **по уровням**: уровень 0 — из `layers` над гридом `map`, каждый надземный — из `levels[n].layers` над гридом `levels[n].map`. Ключи `s0..sN` сквозные по всем уровням (это id экземпляров парта в `GameModel` — два ключа `s0` перетёрли бы друг друга, и половина карты не появилась бы вовсе), и каждый экземпляр получает `level` (номер своего уровня), `solid` (тайлы, блокирующие движение на нём: `physicsStatic` для уровня 0, `levels[n].walls` выше) и `floor` (`levels[n].floor`, на уровне 0 пусто) рядом с `map`, `step`, `layer`, `tiles`, `physicsStatic`, `scale` и `spriteSheet`, которые парт читал и раньше. Карта без `levels` даёт ровно то же, что и прежде.
...
**Пустой каталог — состояние лобби, а не отказ загрузки.** Игры живут в
реестре auth-сервиса, поэтому модератор вправе снять с раздачи последнюю, а на
первом развёртывании не одобрена ещё ни одна. В lobby-режиме бутстрап тогда не
привязывает игру вовсе, а `applyCatalogState`
([client/lib/catalogState.js](../../packages/engine/src/client/lib/catalogState.js)) выключает
«Create server» и печатает `create.emptyCatalogText` в строке отказа лобби —
всё, что от игры не зависит (вход, бейдж пользователя, «My games»,
«Moderation»), остаётся рабочим, и именно оно возвращает каталог к жизни:
«Test» версии активирует её на месте, одобрение приезжает следующей
перезагрузкой вкладки. *Непустой* каталог без единой играбельной строки
(движок обновили, и все опубликованные игры просят возможность, которой в нём
уже нет) — то же состояние лобби: причина от `pickActiveGame` встаёт в ту же
строку вместо `emptyCatalogText`, а не обрывает загрузку. Фатальным «игры нет»
остаётся только для solo и dedicated. Терминальный отказ загрузки
теперь кроет страницу слоем `#tech-informer` (`showBootFailure` в
[views/gameShell.js](../../packages/engine/src/client/views/gameShell.js)), а не заменяет
`document.body`: затирание разметки уносило с собой панель модерации, и
модератор, отключивший последнюю игру, оставался без пути назад.
...
- **предикт** (собственный `client/predictor.rs` игры-плагина, напр.
  `vimp-tanks`'ы): реплика авторитетного движения без
...
- **спавн снарядов** (собственный `client/shot.rs` игры-плагина +
  движковый `client/raycast.rs`): реплика
...
- **`DependencyProvider`** — инъекция сервисов (`renderer`, `soundManager`, `assetsBase`, `localPlayer`) в компоненты по карте `componentDependencies`. `localPlayer` (`{ id, is(id) }`, [lib/localPlayer.js](../../packages/engine/src/client/lib/localPlayer.js)) отвечает, своя ли это сущность: part сравнивает id, полученный четвёртым аргументом конструктора (`{ id }`), со своим gameId, который сервис читает у клиентского ядра лениво. Так игра играет звук только за своего персонажа, а не за каждую сущность на полотне. `assetsBase` — база ассетов активной игры из её манифеста: part, рисующий из файлов-картинок, строит URL сам как `${assetsBase}img/<file>` — тем же способом, каким разрешаются звуки (`${assetsBase}sounds/`). Картинок игры движок не везёт: они едут в пакете плагина (`dist/img/`). Рядом с движковыми сервисами в пуле лежат **игровые**: необязательный `ClientPlugin.hooks.services(core)` возвращает карту, которая подмешивается в пул перед движковыми ключами, — так игра достаёт из part'а своё ядро, а движок не знает, что именно она отдаёт (плагин танков раздаёт так `mapDynamics` — геометрию предсказанной динамики карты, по которой эффект выстрела привязывает осколки к задетому ящику). При совпадении имён движковый ключ побеждает, а сервис, не объявленный никем в `componentDependencies`, просто не выдаётся. Пять движковых имён живут в append-only реестре ([config/clientServices.js](../../packages/engine/src/config/clientServices.js)): имя, которое опубликованная игра написала в `componentDependencies`, работает вечно, а вывод из эксплуатации — это алиас, а не удалённая строка. Поскольку пул отдаёт **запрошенное**, шестой движковый сервис ничего не требует от старых игр: они его не просят и не получают. Незнакомое имя тоже не отказ загрузки — part просто получает `undefined` (ради этого и существует правило C4 у `vimp-contract`: сервис, которого никто не дал, выглядит как чистый холст без единой ошибки).
...
- **Непространственные** (звук своего танка): `registerSound(name, { position, spatial: false })` — источник принадлежит игроку, а не миру. Слушатель стоит в камере, камера — это предсказанная позиция своего танка, поэтому собственный двигатель и собственный выстрел попадают ровно на слушателя: HRTF на нулевой дистанции сворачивает петлю в гребенчатую окраску (её слышно как гул), а не в тишину панорамы. Такому экземпляру `PannerNode` **не создаётся вовсе** — сигнал идёт прямо в gain и сохраняет стерео сэмпла. Howler заводит узел лениво, на первом `pos()`/`pannerAttr(id)`, и заканчивает создание парой `pause()`/`play()`, то есть щелчком, — поэтому, пока источник стоит на слушателе, ни один из этих вызовов не делается. Экземпляр, который уже был панорамирован (побывал в мире), вместо этого возвращается в центр на `equalpower`. Флаг можно поменять позже через `updateSoundData(id, { spatial })` — владелец танка узнаёт, что танк локальный, не в конструкторе. **Мировой** источник, наоборот, получает узел на первом же кадре и держит его: щелчок, который Howler делает внутри `setupPanner`, приходится тогда на начало сэмпла, где он не слышен. Ни дед-зоны, ни порога по дистанции больше нет — позиция считается одной непрерывной формулой (см. ниже), поэтому ничто не переключается между «узла нет, сухое стерео» и «HRTF в крайнем ухе» посреди звучащего сэмпла.
...
### Виртуальная высота и рассеивание

Слушатель всегда стоит в `(0, 0, 0)`, мировой источник задаётся вектором относительно него. Слушатель поднят над плоскостью игры на `virtualElevation`, а внутри `innerRadius` этот вектор плавно гасится через `smoothstep`, поэтому позиция считается **одной непрерывной формулой на каждом кадре** — ни порога, ни ступеньки, ни щелчка:
...
Профиль проекции (`mode`) отображает `(sx, sy, H)` на оси Web Audio. Мировой `x` растёт вправо, мировой `y` — **вниз**, а вектор «вверх» слушателя (`Howler.orientation(0, 0, -1, 0, 1, 0)`) — это +Y, отсюда минус везде, где мировой `y` попадает на ось Y:
...
При `d2d = 0` источник в `topDown` стоит ровно в `(0, -H, 0)`: строго снизу по центру, одинаково в оба уха, без ушной фильтрации. При `dx = 20` и `H = 180` угол `atan(20/180) ≈ 6.3°`, при `dx = 600` — `≈ 73°`. Реальная 3D-дистанция никогда не меньше `H`, поэтому `refDistance` стоит выше неё (`200 > 180`) — источник вплотную к игроку по-прежнему звучит на полной громкости. `equalpower` — дефолт `sideScroller`, потому что «спереди/сзади» в виде сбоку физически бессмысленно, а чистый тембр лево/право важен; `verticalFactor` (`0.2`) занижает вертикаль по той же причине, по которой высота в платформере почти не читается на слух.

**Масштаб сцены делит высоту и радиус рассеивания, но не дистанции затухания.** Третьим аргументом `setListenerPosition(x, y, scale)` приезжает масштаб сцены — `CanvasManager.getCameraZoom()`: это масштаб самого полотна (`currentScale / baseScale`, то есть доля расчётной ширины 1920, которую полотно занимает) **умноженный** на динамический зум камеры, ровно те два множителя, произведение которых и есть `finalScale` полотна. И узкое окно, и отдаление камеры сжимают картинку, поэтому оба обязаны сжать вместе с ней стереобазу — иначе панорама шире того, что видит глаз. А вот `refDistance` / `maxDistance` / `rolloffFactor` — атрибуты самого `PannerNode`: они ставятся один раз через `pannerAttr` на уровне `Howl` и покадрово не пересчитываются. Масштабирование одной только JS-отсечки (в `_updateSpatialSound` и в отборе кандидатов `processAudibility`) открыло бы окно, где движок считает источник слышимым, а узел уже отдал тишину, — это не видно ни в одном тесте и слышно как случайно пропадающие звуки. Поэтому `maxDistance` остаётся в мировых координатах.

### `parts.sounds.spatial`

Блок необязателен: игра, которая ничего не объявила, получает движковые дефолты ([config/clientDefaults.js](../../packages/engine/src/config/clientDefaults.js)) и звучит правильно. Каждый ключ проверяется отдельно и по одному падает на дефолт с предупреждением в консоль, поэтому неверное значение не может сломать аудиоконтекст:
...
parts: {
  sounds: {
    ...
    spatial: {
      ...
      panningModel: 'HRTF',     // 'HRTF' | 'equalpower' (дефолт профиля)
...
**Числа здесь — мировые единицы, а не экранные пиксели.** В менеджер приходят мировые координаты (камера из горячего буфера ядра, `regSound.position` от частей игры), а экранный масштаб — отдельный множитель [CanvasManager](../../packages/engine/src/client/components/model/CanvasManager.js): `currentScale = baseScale * (ширина окна / 1920)`. В `vimp-tanks` (`mapScale 0.3`, `baseScale 5`) одна мировая единица — 5 экранных px, в `vimp-snakes` (`mapScale 1`, `baseScale 1`) — ровно один.

- `innerRadius` — это **габарит игрока в мировых единицах**: всё, что звучит внутри него, плавно сводится к центру и распределяется поровну в оба уха. Танки: корпус `8 × 6` единиц, полудиагональ `≈ 5`. Змейка: `baseRadius 14`. Движковый дефолт `40` верен только для игры, где мировая единица равна экранному пикселю.
- `virtualElevation` калибруется от **видимой полувысоты экрана в расчётном окне (1920×1080)**: `H ≈ (высота полотна / 2) / baseScale`. Больше `H` — мягче панорама вблизи игрока, меньше — агрессивнее разделение по ушам. Для игры с `mapScale 0.3` и `baseScale 5` это даёт `H ≈ 540 / 5 = 108`, а половина экрана по горизонтали — `960 / 5 = 192` единицы, то есть угол `atan(192/108) ≈ 60°` на краю экрана и единицы градусов на дистанции габарита игрока. Калибровать нужно только расчётное окно: на любом другом размере движок пересчитает `H` сам через масштаб сцены выше, поэтому объявленное игрой число остаётся верным и на ноутбуке, и на 4K.

Опечатка в ключе или неизвестный движку `mode` — **молчаливый** откат на дефолт. Статически это ловит только правило **E6** у `vimp-contract`, см. [debugging.md](debugging.md).

### Как часто пишется позиция

Геометрия выше пересчитывается каждый кадр, но до `PannerNode` доходит через два ограничителя, и оба нужны по одной причине: внутри Howler один `pos()` — это три `setValueAtTime` на `positionX/Y/Z` плюс событие `'pos'`, а на 60 Гц и лимите голосов это тысячи записей автоматизации в секунду. WebKit отвечает на такой поток артефактами, клиппингом и обрывами — HRTF там настоящая свёртка на каждый узел, а не дешёвая аппроксимация, — тогда как сама панорама от лишних записей ничего не выигрывает.

- **Порог смещения.** Последняя записанная в узел позиция запоминается на экземпляр; источник, который не сдвинулся дальше порога, не переписывается. Большинство мировых источников неподвижны — горящий остов, эмбиент, брошенный предмет.
- **Гейт частоты.** Позиции пишутся не чаще, чем раз в `1000 / 30` мс, с допуском в две миллисекунды на границе — иначе кадр 60 Гц, пришедший чуть раньше границы, уехал бы на следующий и такт упал бы до 20 Гц. 30 Гц — обычный темп обновления позиций в игровом звуке, и на слух это неотличимо от 60.

**Под гейт попадает ТОЛЬКО запись позиции.** Громкость идёт через `_applyVolume` каждый кадр, как и обновление `rate` и уборка экземпляров, чей источник исчез. Громкость обязана: игра ведёт её от скорости (петля двигателя), и ступенька в 30 Гц была бы слышна, а глушение за `maxDistance` должно срабатывать в том же кадре, в котором источник ушёл за радиус, а не через 33 мс.
