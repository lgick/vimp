...
в футере формы входа, и по тем же правилам стилей — ссылка на страницу проекта
...
пакета движка (`version` плюс `name`/`homepage`) и запекается в бандл на
сборке: version-эндпоинта у мастера нет. Ссылку в обоих футерах строит один
`resolvePackageLink` (`src/lib/packageLink.js`, рендер —
`client/lib/footerLink.js`): он приводит `homepage`/`repository` к https, а при
их отсутствии откатывается на страницу пакета в npm, которой достаточно одного
имени, — поэтому рабочая ссылка есть и у пакета, не объявившего ни того, ни
другого. Крейта
