...
**Зум камеры делит высоту и радиус рассеивания, но не дистанции затухания.** Множитель зума приезжает третьим аргументом `setListenerPosition(x, y, scale)`: при отдалении стереобаза сжимается вместе с картинкой. А вот `refDistance` / `maxDistance` / `rolloffFactor` — атрибуты самого `PannerNode`: они ставятся один раз через `pannerAttr` на уровне `Howl` и покадрово не пересчитываются. Масштабирование одной только JS-отсечки (в `_updateSpatialSound` и в отборе кандидатов `processAudibility`) открыло бы окно, где движок считает источник слышимым, а узел уже отдал тишину, — это не видно ни в одном тесте и слышно как случайно пропадающие звуки. Поэтому `maxDistance` остаётся в мировых координатах.
...
- `virtualElevation` калибруется от **видимой полувысоты экрана**: `H ≈ (высота полотна / 2) / currentScale`. Больше `H` — мягче панорама вблизи игрока, меньше — агрессивнее разделение по ушам. Для игры с `mapScale 0.3` и `baseScale 5` в окне 1920×1080 это даёт `H ≈ 540 / 5 = 108`, а половина экрана по горизонтали — `960 / 5 = 192` единицы, то есть угол `atan(192/108) ≈ 60°` на краю экрана и единицы градусов на дистанции габарита игрока.
...
- **Гейт частоты.** Позиции пишутся не чаще, чем раз в `1000 / 30` мс: 30 Гц — обычный темп обновления позиций в игровом звуке, и на слух это неотличимо от 60. Под гейт попадает только позиция: уборка экземпляров, чей источник исчез, и обновление `rate` по-прежнему идут каждый кадр.
