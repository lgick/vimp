...
- **CanvasManager** — управляет несколькими PixiJS `Application` одновременно: `vimp` (основной игровой canvas) и `radar` (мини-карта); canvas-элементы генерирует `main.js` из конфига канвасов игры (`modules.canvasManager.canvases`, включая стартовые `width`/`height`) — в HTML их нет. Адаптивное масштабирование (эталон 1920px), `aspectRatio`/`fixSize`/`baseScale`, динамическая камера (look-ahead, zoom от скорости) и тряска — параметры в [configuration.md](configuration.md#modulescanvasmanager--полотна-и-камера). Кадр камеры только ДВИГАЕТ сцену (`updateCoords`: `stage.position`, `stage.scale`) и ничего не рисует — полотно рисует тикер (`TickerPlugin` держит `app.render` на `Ticker.shared` с приоритетом LOW, то есть после `renderTick`), раз за тик и по кадру, применённому целиком. Своя отрисовка означала бы отрисовку на каждый кадр КАМЕРЫ, а за тик их несколько (сперва камера дискретного кадра, следом предсказанная): два-три полных обхода сцены на один видимый кадр, и каждый со своим `onRender` у всех партов по наполовину применённому кадру. Единицы и `updateCoords`, и `toWorld` — `renderer.screen`, то есть те, в которых живёт трансформ сцены, а не пиксели буфера полотна: совпадают они, только пока `resolution` рендерера равен единице, и расхождение было бы тихим (картинка осталась бы на месте, а центр камеры, который игра восстанавливает по трансформу сцены — проекция высоты 2.5D в `vimp-tanks`, — уехал бы на полэкрана).
