...
- **Пробуждение вкладки** (`visibilitychange` → visible): помимо размьюта оболочка зовёт `clientCore?.resync?.()` — часы интерполятора пересеиваются со следующего кадра, а не догоняются EMA. Вызов опциональный: у старой сборки плагина такого метода ABI нет.
- **Потеря WebGL-контекста** (`webglcontextlost` на каждом полотне): весь видимый контент — GPU-only `RenderTexture` без CPU-источника, поэтому при потере контекста сцена рисовалась бы пустой. Обработчик зовёт `preventDefault()` (без него браузер не пришлёт `webglcontextrestored`) и снимает `renderTick` с тикера. На `webglcontextrestored` оболочка удаляет все контроллеры (они держат мёртвые текстуры), перепекает ассеты (`BakingProvider.bakeAll` в тот же экземпляр `Map`, который держит `GameModel._assets`, — старые render-текстуры перед этим уничтожаются), пересобирает карту из сохранённого payload'а `MAP_DATA` **без** повторного `MAP_READY` (хост его больше не ждёт) и возвращает `renderTick`. Танки и динамика восстановятся сами из ближайших кадров.
...
слева, вкладки Active Servers / Leaderboard справа.
...
- **`reset()`** останавливает все звучащие инстансы, но **сохраняет
  ...
  `processAudibility()`. Реестр целиком чистит только `destroy()`.
