...
— группа стражей рампы: коллайдеров по обоим бортам прогона и поперёк его
дальнего торца, которые `GameMap::create_static` строит только для слоёных
карт. Фильтр тела собирает `body_filter(mask, on_ramp)`
...
- Клиентская половина: `trait GameClientDef` — `new`, `on_server_state`,
  ...
  же, как их разрешает хост: `obb_vs_obb` / `collect_tile_contacts` дают
  `Contact` (геометрию они читают тем же `Box2` и той же тайловой сеткой,
  что и `raycast`, — луч и контакт не могут разойтись в том, где стена),
