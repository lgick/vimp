...
Карта может нести надземные уровни рядом с землёй. Уровень 0 остаётся в
полях `map` / `physicsStatic` / `layers`, поэтому карта без новых полей
грузится ровно как раньше. Второй уровень приходит необязательным полем
`levels`, переходы между уровнями — необязательным полем `ramps`:
...
* `dir` — направление *подъёма* (`north` = `-y`, `south` = `+y`,
  `west` = `-x`, `east` = `+x`); `from`/`to` по умолчанию `0`/`1`.
...
* **край плиты либо закрыт перилами, либо куда-то приземляет** — клетка
  `floor`, у которой сосед не плита и не перила, это обрыв, и падение с
  него обязано приземлиться на проходимую землю уровня 0; за границей грида
  или над стеной это ошибка (стены уровня 0 танку на плите не преграда).
...
тело в `GROUP_1` (земля) или `GROUP_2` (эстакада) и оставляет ему видимым
только свой уровень. Тело, не выставившее группы, остаётся в `Group::ALL` и
продолжает взаимодействовать с уровнем 0 — одноуровневые миры и игры, не
знающие про уровни, ничего не замечают.
...
рёбра рамп и односторонние рёбра обрывов (только сверху вниз и со штрафом —
прыжок стоит игрового `fallDamage`). Пути ищутся через
