...
- **Какие игры-плагины есть на машине** — по симлинкам `npm link`,
  ...
  валидируется (scope, зависимость `vimp-engine`, скрипты
  `build`/`core:build`, `vimp-engine-core` в `core/Cargo.toml`, чистое
  git-дерево с remote) и подтверждается по одному.
...
| `--relink` | только вернуть локальные линки и выйти (после `SIGKILL`) |
