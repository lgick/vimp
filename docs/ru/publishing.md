...
- **Какую версию предложить** — по под-заголовкам `[Unreleased]`:
  `### ⚠️ Breaking` → minor в `0.x` (major начиная с `1.0`), `### Added` →
  minor, иначе patch. Enter принимает, либо вводится
  `patch`/`minor`/`major`/своя версия. У игр CHANGELOG нет, поэтому там
  предложение опирается на бамп крейта/`ENGINE_API_VERSION` и всегда
  подтверждается.
