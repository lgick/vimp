...
- **Какие артефакты публиковать** — по трём независимым сигналам:
  ...
  (`npm view`, `index.crates.io`) и непустая секция `## [Unreleased]`. Затем
  применяются правила распространения из таблицы ниже: публикация крейта
...
| `--yes` | принять предложенные версии; пуш в `main` всё равно спрашивается |
