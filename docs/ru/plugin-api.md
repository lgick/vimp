...
загрузчик проверяет это и называет нарушенный контракт; заодно импортируемые
половины плагина сверяются с `engineApi` манифеста (пересобранный манифест
рядом с отставшим `dist/` — это отказ, а не зелёный прогон).

**Половина, грузимая в Node, обязана быть самодостаточной.** Внешние
зависимости — в первую очередь `pixi.js` — допустимы только в клиентской
половине и только в браузере, где их резолвит import map (правило контракта
A1 держит `pixi.js` вне `dependencies`, чтобы движок и игра делили один
экземпляр PixiJS). Поэтому клиентскую половину Node импортирует **по
требованию**: dedicated-сервер её не касается вовсе (браузер получает её
статикой), а headless-раннер, которому она нужна виртуальным клиентам,
просит её сразу. Host-половина и `entries.wasmNode` грузятся в Node всегда,
и неразрешимый импорт в них роняет старт именованным отказом с именем entry
и ненайденного пакета, а не сырым `ERR_MODULE_NOT_FOUND`.
...
**Эти URL-ы мастер переписывает в отдаваемом манифесте.** Сборка игры не
знает, по какому адресу будет раздаваться именно эта её версия, а мастер
держит на диске несколько версий одной игры сразу и раздаёт их по
`/games/<id>/<version>/`. Поэтому `rebaseManifest` переносит `assetsBase` и
`entries.client`/`host`/`wasm` на эту версионную базу и **добавляет
`mapsBase`** (`/games/<id>/<version>/maps`) — именно из этого поля лобби
берёт адреса карт, а при его отсутствии откатывается на `/games/<id>/maps`
(dev, standalone, dedicated на пути `node_modules`). Приём тот же, каким в
dev entries уже подменяются на исходники Vite `/@fs/`.

Два следствия для автора игры:

- **`entries.wasmNode` не переписывается никогда** — это путь внутри `dist/`
  для Node, а не URL;
- entry, лежащий **не** под собственным `assetsBase` манифеста (например,
  абсолютный URL на ваш CDN), остаётся ровно таким, как записан, а не
  «чинится»; пакет с испорченным манифестом отсеивается на приёме
  структурной проверкой мастера, которая код плагина не исполняет.
...
сначала разрешает алиасы выведенных `control` — ровно как билдер формы,
чтобы поле проверялось тем контролом, которым построено, — а затем проверяет,
до JS-валидатора:

- `control`, которого реестр не знает вовсе: `unknown control`. Билдер такое
  поле не строит, значит от честного клиента оно и не приезжает; статически
  это ловит `C10`.
- **числовое** text-поле (`numeric: true` или `unit`) — отдельной веткой,
  потому что форма отдаёт его *числом*: `must be a number`, затем `min`/`max`
  в единице **отображения** (`toDisplay`, общий с билдером). `regExp` здесь
  намеренно **не** применяется: клиент сверяет паттерн с сырой строкой,
  которую набрал игрок, а до хоста доезжает только число, и обратная
  сборка строки отбила бы каноническую запись, которую форма приняла
  («1.50» приезжает как 1.5), — вход оказался бы заперт, потому что строки,
  годной обеим сторонам, не существует.
- иначе: длину (`too long`), членство в объявленном списке `options` поля
  `select`/`radio` (`not an option` — браузер другого значения и не отправит,
  значит не должен и обошедший форму клиент), затем `regExp`
  (`invalid format`, якорится ровно так же, как в браузере). `maxlength` и
  `regExp` применяются только к текстовым полям — ровно как в форме.

Поле `select`/`radio`, у которого список `options` пуст, отсутствует или
заменён на `source`, не принимает **ничего**: auth-форма строится без
`ctx.sources`, поэтому такое поле резолвится в пустой список, показывает
`no options available` и заполнить его нельзя вовсе — пропустить его на хосте
значило бы дать обошедшему форму клиенту то, чего у игрока нет. `C10`
запрещает `source` в `authSchema`; в `roomForm` (правило `B5`) он остаётся
штатным способом отдать каталог карт.
...
Два осознанных пробела. `required` на хосте **не** проверяется: solo-путь
...
игрового валидатора. А `regExp`, который не компилируется, — дефект схемы, а
не ограничение, и поле проходит (как и на клиенте).
...
export default {
  ...
  serviceNames: ['mapDynamics'],   // необязательное: что вернёт services()
};
```
...
`serviceNames` — тот же список, записанный статически. В рантайме его никто
не читает: он нужен контрактному чекеру, который не может позвать
`services(core)` (для этого нужно живое ядро) и потому не отличает игровой
сервис от опечатки в `componentDependencies`. Без списка правило `C4`
сообщает о незнакомом имени предупреждением, со списком — снова ошибкой; а
сервис, которого никто не дал, молча приходит в part как `undefined`.
...
уровня строкой (`"1"` — первый надземный; уровень 0 остаётся в
`map`/`physicsStatic`/`layers`), значение — `{map, floor, walls, layers}`:
грид той же размерности, что и `map`, тайлы, по которым можно ездить,
тайлы, блокирующие движение (перила), и рендер-слои этого грида. `ramps` —
список `{tile, dir, from, to}`, направленные тайл-переходы между уровнями
(`dir` — направление подъёма: `north`/`south`/`west`/`east`). Точка респауна
может нести четвёртым числом свой уровень (`[x, y, angle, level]`), объект
`physicsDynamic` — поле `level`; по умолчанию и то, и другое — земля. Весь
формат проверяется в ядре (`MapConfig::validate`), а до сборки — правилом
контракта **E4**. Игра, которая без него не работает, объявляет `requires:
['map.layers']`, а игра, которой нужно больше двух уровней, — ещё и
`map.levelsN`.

`levelHeight` — высота ОДНОГО уровня в мировых единицах (без масштаба, как
и `step`: ядро умножает её на масштаб карты само). Поле необязательное, по
умолчанию — размер тайла, то есть подъём «уровень на тайл» даёт уклон 1.0.
Оно существует потому, что всё, кроме номера уровня, меряется в мировых
пикселях: без него `RampSample::slope` был бы «уровней на пиксель», и любая
константа, подобранная под безразмерный уклон (тяга в горку, наклон
...
торца (`RAMP_GUARD_GROUP`). Тело уровня, с которого прогон начинается, их
видит (заехать на клин сбоку или под него с «неправильного» торца нельзя),
тело, законно поднимающееся по прогону, — нет
(`levels_interaction_on_ramp`), тело другого уровня не встречает их вовсе.
Подножие прогона не закрывается никогда — это законный вход. Широкая
рампа режется на параллельные полосы-прогоны, но стражи огораживают **блок**
полос, а не каждую полосу: борта ставятся только по внешним границам блока,
а торцевой страж — один, во всю его ширину. Полосы одного блока несут общий
`RampRun::block` — по нему игра отличает смену полосы от новой горки. Страж
на полосу превратил бы широкую рампу в набор жёлобов шириной в тайл и
останавливал бы тело, входящее по границе полос: подниматься оно ещё не
начало и стражей видит. Клетки двух
разных рамп пересекаться не имеют права: общая клетка досталась бы одному
из прогонов произвольно.

Уровней бывает до `MAX_LEVELS` (8): земля и семь надземных, у каждого свой
бит маски столкновений, — и рампа вправе перешагивать через несколько
уровней. У любого уровня (включая землю) может быть необязательное поле
`volumes` — словарь `{ "<ключ layers>": высота }`: визуальная высота
рендер-слоя в уровнях. Ядро её не использует (поле едет клиенту, геометрию
строит рендер игры), но проверяют обе стороны: ключ обязан называть
рендер-слой того же уровня, высота — лежать в `(0, 8]`. У динамических тел
карты появились собственные правила уровня: тело, под которым не осталось
плиты, падает на ближайшую плиту снизу (`landing_level`) по той же
траектории (`FallModel`, `gameConfig.mapFallTime`, по умолчанию 0.35 с на
уровень), что и всё, что роняет игра.

На клиенте те же `levels` доезжают до рендер-партов: `applyMapData` собирает
...
уходит); `{handled: true, bytes}` — ответ. Любая другая форма ответа (нет
самого метода `dispatch`, `undefined`, строка) читается тем же «не
обработан»: ядру доверия ровно столько же, сколько его самоописанию. Первый
опкод, `debug.json`, дублирует замороженный метод `debug_json`: вызывающий
пробует опкод и откатывается на метод — так дамп получает ядро любого
возраста. Обе половины остаются навсегда: И1 ничего не удаляет.

Алиасы опкодов резолвятся в сторону, **обратную** остальным реестрам, и это
намеренно. Имя контрола или сервиса пишет *игра*, а движок разрешает его
вперёд; имя опкода пишет *движок*, а понимать его должно уже опубликованное
ядро, которое движка старше. Поэтому `dispatchCoreOp` пробует по
`abi_describe` всю цепочку: активное имя первым, выведенные — запасным путём
для старого ядра. Резолв только вперёд молча терял бы возможность ровно на
тех ядрах, ради которых механизм и заведён.
...
профиль rank/state.) `SNAPSHOT_FORMAT_VERSION` остаётся 5 (фрейминг
...
(срезы рейтинга), `accolades` (порт `ACCOLADES_DATA` и клиентский сервис),
`dispatch` (`dispatch`/`abi_describe` в ядре), `map.layers` (слоёные 2.5D-
карты: `levels`/`ramps` в формате карты, маски уровней в физике,
`set_actor_level`, слоёный нав-граф) и `map.levelsN` (больше двух уровней,
...
нет, а значит нет и манифеста, из которого можно взять `requires`.
`startStandaloneGame({ requires })` перекрывает обе половины, если
встраивающий код знает лучше.

Три списка **обязаны совпадать**: игра, чей манифест просит возможность,
которую половины не объявили, отвергается лобби и принимается SDK, после чего
тихо недоигрывает — именно тот молчаливый режим, ради отказа от которого
возможности и заводились. Расхождение отвергает правило контракта **B2** —
там его и встречает автор игры, до публикации. `loadGamePackage` только
*предупреждает*: Node-пути читают авторитетный манифест, чья совместимость уже
проверена, а пакет, который грузился раньше, обязан грузиться и дальше (И4).
Имена сравниваются **резолвнутыми** по реестру возможностей, поэтому манифест
с новым именем и половина, оставшаяся на выведенном алиасе, согласованы.
Половина, вовсе не объявившая `requires`, из сверки исключена;
`requires: []` — нет, это уже утверждение, что игре ничего не нужно.
...
несовместимая игра, а не роняет того, кто его прочитал. То же и для половин:
SDK склеивает их через `mergeRequires`, который недоверенное значение никогда
не спредит, — иначе голая строка разошлась бы списком отдельных букв.
...
| `SNAPSHOT_FORMAT_VERSION` (=5) | движок (фрейминг) | схема блоков едет в CONFIG_DATA → внутри комнаты всегда согласована |
