...
На стороне JS единственный разрешённый способ позвать необязательную
возможность ядра — `GameCoreAdapter._op(op, payload)`: он не зовёт опкод,
которого загруженное ядро не объявило. Обращение к `this._core.<имя>` вне
замороженного списка запрещено правилом ESLint. Первый опкод, `debug.json`,
дублирует замороженный метод `debug_json`: адаптер пробует опкод и
...
| --- | --- |
| каталог мастера (`GameCatalog`) | игра **остаётся** в `manifestList` с полем `compat: {ok: false, missing, text}`; лобби показывает её недоступной с причиной, комнату по ней создать нельзя |
...
| standalone SDK | бросает на любой из половин плагина |
