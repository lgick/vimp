...
setId, physicsStatic, physicsDynamic, levels, ramps}`: сырые поля MAP_DATA
...
`solid` (блокирующие тайлы), `floor` и `volume` (высоту слоя из `volumes`,
`0` — слой плоский) — см.
...
`z`/`level` в строке динамики, `volumes`). Зарегистрированное имя
