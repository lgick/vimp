...
```jsonc
{
  ...
  "packageVersion": "0.13.1",              // ОПЦИОНАЛЬНО: npm-версия пакета игры
...
`packageVersion` **опционально** и носит справочный характер: это semver
собственного npm-пакета игры, который `build-game-manifest.js` копирует из её
`package.json`. `version` выше — хеш содержимого: он говорит движку, не устарел
ли бандл, и игроку ничего не значит, поэтому форма входа (`#auth`) показывает в
футере именно `packageVersion`. Игра, собранная до появления поля, его не
отдаёт — строка футера остаётся пустой; больше поле в движке никто не читает.
