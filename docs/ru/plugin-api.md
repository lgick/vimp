...
`solid` (блокирующие тайлы) и `floor` — см.
...
`dispatch` (`dispatch`/`abi_describe` в ядре) и `map.layers` (слоёные 2.5D-
карты: `levels`/`ramps` в формате карты, маски уровней в физике,
`set_actor_level`, слоёный нав-граф). Зарегистрированное имя
