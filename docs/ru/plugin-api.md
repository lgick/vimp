...
После первого сабмита правка любого поля **перепроверяет форму**, а не
гасит блок: строка уходит, когда починено именно её поле, — список того,
что ещё не так, переживает первое нажатие клавиши. До первого сабмита не
показывается ничего: игрок ещё заполняет форму.
