...
- **числовое** text-поле (`numeric: true` или `unit`) — отдельной веткой,
  ...
  в единице **отображения** (`toDisplay`, общий с билдером), затем `regExp`
  по тому же отображаемому значению — паттерн ловит то, что диапазон не
  ловит (дробное, ведущий ноль).
...
Три списка **обязаны совпадать**, и это проверяют `loadGamePackage` и правило
контракта **B2** — ровно так же, как они проверяют согласованность
`engineApi`: игра, чей манифест просит возможность, которую половины не
объявили, отвергается лобби и принимается SDK, после чего тихо недоигрывает —
именно тот молчаливый режим, ради отказа от которого возможности и заводились.
Половина, вовсе не объявившая `requires`, из сверки исключена (пакет,
собранный до появления поля, обязан грузиться); `requires: []` — нет, это уже
утверждение, что игре ничего не нужно.
