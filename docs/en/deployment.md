...
`add-server.sh` (both share `lib/common.sh`, so Nginx, SSL and the project
folder are literally the same code):
...
(below). The box's `.env.prod` and `docker-compose.yml` are written by the
first CI deploy — until then nothing is running on the port. The script ends
by printing the `SERVERS_MATRIX` entry to add.
...
- `settings` is an optional field holding the room overrides (`map`,
  ...
  span lines.
  Which fields mean anything depends on the game — one without maps or
  timed rounds has no use for `map` or `roundTime`; see the game's own
  configuration docs.
  **The matrix is the source of truth**: `.env.prod` is regenerated on every
  run, so a line added on the box by hand survives a deploy only if the
  matrix carries it too. `settings` may
