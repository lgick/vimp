...
1. Upload the scripts to the server:
   ...
   ```bash
   scp -r .github/deployment/* root@YOUR_SERVER_IP:~/vimp-deployment-scripts/
   ```

   Copy the whole directory, not just `*.sh`: the wizards `source`
   `lib/common.sh` and refuse to start without it.
...
## Step 3b: Adding a dedicated server

A [dedicated box](dedicated.md) — one 24/7 match of one game, no lobby and
no OAuth — is set up by its own wizard, `./add-dedicated.sh`, next to
`add-server.sh` (both share `lib/common.sh`, so Nginx, SSL and origin
validation are literally the same code):
...
| Auth-service URL | **required** — without it the CSP blocks the lobby login | **optional** — only a game named by registry **id** needs it; a game named by npm package is fetched from npm |
| Game (`@vimp-games/tanks`, `tanks`, either with `@<version>`) | not asked | **required** |
| Room settings (map, player cap, round time, friendly fire) | not asked | asked, each with a default |
| Files written | project folder, Nginx + SSL (an auth domain also gets `.env.prod`, `docker-compose.yml`, `.keys/`) | project folder, Nginx + SSL, `.env.prod`, `docker-compose.yml` |

The generated `.env.prod` carries `VIMP_DEDICATED_GAME`, `VIMP_GAMES_DIR`,
`VIMP_DEDICATED_ROOM` (one line of JSON, no outer quotes — that is how
`env_file` reads it) and `VIMP_AUTH_SERVICE_URL` only when a URL was given.
The generated `docker-compose.yml` mounts the same `vimp-games` volume the
lobby master uses and publishes the port on `127.0.0.1` only.

Both files are regenerated by every CI deploy from `SERVERS_MATRIX`, so the
matrix stays the source of truth — the script's job is to make the box
runnable before the first deploy (`docker compose pull && docker compose up
-d` in the project folder).
...
master: Steps 1–2 above (DNS, `install-system.sh`), then Step 3b
(`add-dedicated.sh` — a regular domain with its own port), then one extra
field in `SERVERS_MATRIX`:
...
`install-system.sh` → `add-server.sh` (or `add-dedicated.sh` on a dedicated
box) for each domain, then
