...
| E1–E4 | assets and maps: the `webm` + `mp3` pair per sound, map images in `dist/`, an empty sound registry (warning), and the structure of a layered (2.5D) map — level keys, grid dimensions, walls inside the floor, ramp tiles and directions, respawn and `physicsDynamic` levels, tiles named by a level's render layers |
