...
games:           id, package_name, title, repo_url, author_user_id, status,
                 ...
                 moderator_user_id, deleted_at, created_at,
                 updated_at                                   ← game registry
...
**Deleted games.** `games.deleted_at` (migration `012_games_soft_delete.sql`)
is what deletion actually writes: the game leaves the players' catalog at once,
but its row and everything keyed by its `game_id` stay put. A separate column
rather than a fifth status, because the status it had — `pending`, `approved`,
`rejected`, `disabled` — is where `POST /admin/games/:id/restore` puts the game
back, and a status overwritten by `deleted` would have nothing to restore to.
`packages/auth/src/db/gamesPurgeJob.js` is the daily job that finishes the
deletion at **00:15 UTC** (ten minutes after the ratings run, to keep the two
apart in the log): everything with `deleted_at` older than
`config.games.deleteRetentionDays` (30) days loses its rows in `rank_periods`,
`rank_events`, `state_snapshots`, `states`, `ratings` — in that order, the
`games` row last — and is gone. None of those tables has an FK on `games`, so
the cleanup is explicit: orphaned rows would come back to life under a later
submission that took the same id. The run is not wrapped in a transaction (this
repository holds none anywhere), so the order makes an interrupted run safe to
repeat, and it takes the same kind of **session-scoped advisory lock** as the
ratings job, on its own key. `npm -w @vimp/auth run db:games-purge` runs it by
hand. There is deliberately no route that empties the `Deleted` tab early: the
window has to be the same for everybody.
...
| `POST /games/:id/version` (Bearer — author or admin) | `{ version }` → `{ game }`: stages a new version for review without touching the served `version`, clears `moderatorNote` and lifts `rejected` back to `pending`. `404 unknownGame`, `403 forbidden` for someone else's game . A deleted game answers `404 unknownGame`: writing into its row would make a later `Restore` bring back a different game than the one that was deleted |
| `DELETE /games/:id` (Bearer — author or admin) | **soft** removal: the row gets `deleted_at = now()` and nothing else is touched. The game disappears for players at once — `GET /games` stops listing it and every master drops it from its catalog on the next sync — while its row and every row keyed by its `game_id` (`rank_periods`, `rank_events`, `state_snapshots`, `states`, `ratings`) stay in the database for `config.games.deleteRetentionDays` (30) days. Until then `POST /admin/games/:id/restore` brings the game back whole; after them the daily purge job removes it for good. Deleting an already deleted game just moves `deleted_at` — the retention window starts over. The `id` and the package name stay taken for the whole window (a later submission under the same id answers `409 gameExists`): restoring has to be guaranteed. An admin may delete a game in any status; an author only their own, and only while it is not being served: `409 gamePublished` for an `approved` game (an admin disables it first), `403 forbidden` for someone else's, `404 unknownGame` if there is none **or it is already deleted** — a stale panel card must not silently restart the 30 days. When an admin's deletion leaves the platform with no servable game, the answer carries `warning: 'catalogEmpty'`, exactly as a moderator's decision does. Rate-limited per IP with the other registry writes (5/60s) |
| `GET /admin/games` (Bearer, admin) | the whole moderation queue, freshest first, with author and moderator nicks — **including soft-deleted games** (each carries `deletedAt` and `purgeAt`, the date it will be removed for good): the panel's `Deleted` tab is drawn from this same answer |
| `POST /admin/games/:id/restore` (Bearer, admin) | brings a soft-deleted game back together with its ratings and skills — clears `deleted_at`, the status it had before deletion is still there. Admin only: an author never sees their deleted game (`GET /games/mine` filters it out), so there is nowhere for them to restore it from. `404 unknownGame` if there is no such game or it was not deleted |
| `PATCH /admin/games/:id` (Bearer, admin, `{ status?, version?, pendingVersion?, note?, maxGameScore?, authorNick? }`) | a moderator's decision — only the keys present are written. `status: 'approved'` without an explicit `version` promotes `pendingVersion` to `version` and clears it (the common path must not need two fields). `maxGameScore` must be a positive integer within this service's own ceilings (`<= config.rank.maxGameScore`, and `× 20` — the engine's merge window — still within `config.rank.maxPoints`), else `400 invalidMaxGameScore`: a ceiling above them would let the master forward a result this service then rejects, and the host would retry that flush forever. `404 unknownGame`. **`authorNick`** reassigns authorship: the nick is resolved to a user id case-insensitively (`400 badRequest` if it isn't a valid nick, `404 unknownUser` if nobody holds it), and `null` or `''` clears the author — a game of the platform's own is legitimately nobody's. It is the only way to give an author to the games seeded by `009_games.sql` with `author_user_id = NULL`: until an admin does it, they are in nobody's "My games" and nobody can request a new version for them. When the decision leaves the platform with no servable game at all, the answer carries `warning: 'catalogEmpty'` — not a refusal (the lobby stays up: sign-in, "My games" and "Moderation" do not depend on a game), but rooms cannot be created until something is published again, and the moderator has to learn that here rather than from the players. A deleted game answers `404 unknownGame` for the same reason its author cannot submit a version for it |
