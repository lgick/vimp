...
| `POST /games` (Bearer, `{ id, packageName, title?, repoUrl?, version }`) | a developer's submission: `201 { game }`, created as `pending` with `pendingVersion = version`. Rate-limited per IP (5/60s). A missing `id`, `packageName` or `version` is `400 { error: 'badRequest', field }` — presence belongs to this route, while the format checks below are shared with moderation. Each field answers with its own code — `400 invalidGameId` \| `invalidPackageName` \| `invalidVersion` \| `invalidTitle` \| `invalidRepoUrl` — `409 gameExists` for a taken `id` or an already registered package (both are the same `23505`), `403 tooManyGames` above `config.games.maxPerUser` |
