...
44. Тайлы: какой размер тайла (`step`) и какие индексы делают стену
    (`physicsStatic`)? **Учтите ловушку:** картинки тайлов грузятся из
    каталога движка `public/img/` (`tiles.png`, `tiles2.png`, `tiles3.png`,
    `b1.png`), а не из ассетов вашего пакета. Берём готовые или нужны свои?
...
46. Нужны ли динамические объекты (ящики, бочки)? Для каждого: позиция,
    размер, плотность, картинка, слой, демпфирование.
