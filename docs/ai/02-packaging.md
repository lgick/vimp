...
```json
{
  ...
  "roomForm": [
    { "name": "maxPlayers", "control": "text", "label": "Max players", "numeric": true, "regExp": "[1-8]" },
    { "name": "roundTime", "control": "text", "label": "Round time", "unit": "s", "numeric": true, "regExp": "…" },
    { "name": "mapTime", "control": "text", "label": "Map time", "unit": "s", "numeric": true, "regExp": "…" },
...
Numeric bounds are not min/max attributes (engine API v3 forms have no
`min`/`max`/`step`); the generator converts the real clamp bounds into an
exact regex with `rangeToPattern(min, max)` and injects it as `regExp` →
rendered as the input's `pattern`. Native validation is a UX hint only — the
authoritative clamp happens on the host.

Form control set in v3 is exactly: **`text`, `select`, `checkbox`, `radio`**.
Descriptor fields: `name`, `control`, `label`, `default`, `hidden`, `numeric`,
`unit: 's'` (stored in ms, displayed in seconds), `regExp`, `required`,
`maxlength`, `options` (`[value]` or `[{value,label}]`), `source: 'maps'`
(room form only), `storage` (auth form only, persists to `localStorage`),
`validator` (auth form only, host-side). An unknown `control` makes the engine
skip the field with a `console.error`.
