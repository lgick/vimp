...
```json
{
  ...
  "entries": {
    ...
    "wasmNode": "../core/pkg-node/vimp_tanks_core.js"
  },
...
| `entries.wasmNode` | **optional**, and unlike the others a *filesystem path relative to the manifest*, not a URL: the `--target nodejs` build of the same core (`core/pkg-node/<crate>.js`). The browser never reads it; it is what lets the engine's headless runner (`npm run sim`) simulate your game on its real core — see `13-debugging.md`. Omit it and you can still pass `--core <path>` by hand |
...
| `build-game-manifest.js` | hashes the bundles, collects map names, applies `rangeToPattern` to numeric room fields, writes `dist/manifest.json` |
