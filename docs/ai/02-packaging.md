...
| `engineApi` | always `ENGINE_API_VERSION` imported from the engine, never a literal |
| `version` | `sha256(sha256(client) ‖ sha256(host) ‖ sha256(wasm))`, first 16 hex chars — clients compare it to detect a stale bundle |
| `packageVersion` | **optional**, informational: the semver of your own `package.json`, copied in by `build:manifest`. `version` above is a content hash and means nothing to a player, so the entry form (`#auth`) shows this in its footer. Omit it and the footer line stays empty |
...
`manifest.json` is missing, `manifest.id` ≠ configured id, or
`manifest.engineApi` ≠ the engine's `ENGINE_API_VERSION`. Skipped means
