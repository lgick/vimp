...
| `requires` | **optional** array of engine capability names the game cannot run without (e.g. `["accolades"]`), checked against `vimp-engine/src/lib/capabilities.js`. Omit it unless the game truly breaks without the feature — a game that degrades gracefully needs nothing. It must be an array of strings: any other shape is read as a broken manifest and the game is treated as incompatible. Declare the same list on `HostPlugin.requires` and `ClientPlugin.requires` too — the standalone SDK (`npm run dev`) has no manifest and reads it from the halves. The three lists must **agree**: contract rule `B2` refuses a package whose manifest and halves disagree, and `loadGamePackage` warns about it (a half that declares no `requires` at all is exempt; `requires: []` is not; names are compared resolved through the capability registry) |
